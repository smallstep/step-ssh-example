...
This repo contains a showcase of how to use SSH certificates (for hosts & users) generated by [`step-ca`](https://github.com/smallstep/certificates) using the [`step CLI's`](https://github.com/smallstep/cli) `ssh` sub-command.
...
v0.13.3+ ([installation docs](https://github.com/smallstep/cli#installation-guide))
and [Vagrant](https://www.vagrantup.com/docs/installation/) (plus a provider like
[VirtualBox](https://www.virtualbox.org/)) installed locally.

## Setup VM

We're going to run a CA in your local environment, and we'll use `ssh` to
connect to a Vagrant VM
(representing a remote host) that has `sshd` pre-configured to accept
...
This example repo includes a pre-generated SSH host certificate and key. To replace it
or generate SSH certificates for other hosts running following command:
